...
export default function RegistrarProducto() {
  ...
  const verificarProveedor = () => {
    ...
    if (isNaN(id)) {
      setError('Ingrese un ID de proveedor válido.');
    ...
    if (prov) {
      ...
      setError('Proveedor no encontrado. Verifique el ID.');
      setProveedorValido(null);
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!proveedorValido) {
      setError('Debe verificar un proveedor primero.');
      return;
    }
    if (!codigo || !nombre || !precioCosto || !precioVenta || !stockActual || !stockMinimo) {
      setError('Complete todos los campos obligatorios.');
      return;
    }

    const existe = productosMock.find(p => p.codigo === codigo);
    if (existe) {
      setError('El código de producto ya existe. Ingrese un código distinto.');
      return;
    }
    ...
    const sm = parseInt(stockMinimo);
    if (pc <= 0 || pv <= 0 || sa < 0 || sm < 0) {
      setError('Verifique que los valores numéricos sean correctos.');
      return;
    }
    ...
    productosMock.push({
      ...
      stockActual: sa,
      stockMinimo: sm,
      idProveedor: proveedorValido.id,
    });
  ...
  const proveedorNombre = (id: number) =>
    proveedoresMock.find(p => p.id === id)?.nombreEmpresa ?? '—';
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            {verProductos ? 'Ocultar catálogo' : 'Ver catálogo de productos'}
          </button>
        </div>
        ...
        <div className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
          <label className="block text-sm font-medium text-slate-700 mb-2">
            Verificar Proveedor <span className="text-red-500">*</span>
          </label>
          ...
          {proveedorValido && (
            <div className="mt-2 text-sm text-green-700 bg-green-50 px-3 py-2 rounded">
              Proveedor válido: <strong>{proveedorValido.nombreEmpresa}</strong>
            </div>
          )}
          <div className="mt-2 text-xs text-slate-400">
            IDs: 1=Distribuidora Norte, 2=Faber-Castell, 3=Papelera del Valle, 4=Maxim Oficinas
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="button"
              onClick={() => { setCodigo(''); setNombre(''); setCategoria('Escolar'); setPrecioCosto(''); setPrecioVenta(''); setStockActual(''); setStockMinimo(''); setProveedorId(''); setProveedorValido(null); setError(''); setSuccess(''); }}
      ...
      <div className="text-xs text-slate-400 text-center">
        Contrato: registrarProducto(datosProducto) — UC-03 Registrar Producto
      </div>
    </div>
  );
}
